...
void main() {
  ...
  group('Product Listing Screen Tests', () {
    test('C73420: Filter - SelectAll Lawn Goals', () async {
      //Quiz
      ...
      //Tap on Lawn Goals button
      await mainProductListingScreen.clickOnLawnGoalsButton();
      for (var category in mainProductListingScreen.lawn_goals_menu) {
        //Click on Category
        await mainProductListingScreen.clickOnLawnGrowMenu(category);
        // Verify the Page content
        await productListingScreen.verifyProductScreen(category);

        //Verify Clear-Filter Functionality-GrassType
        await productListingScreen.verifySelectAllFilterFunctionality(
            productListingScreen.grass_type_select_deselect_all_button);

        //Verify Clear-Filter Functionality-Sunlight
        await productListingScreen.verifySelectAllFilterFunctionality(
            productListingScreen.sunlight_select_deselect_all_button);

        //Verify Clear-Filter Functionality-TypeOfControl
        await productListingScreen.verifySelectAllFilterFunctionality(
            productListingScreen.type_of_control_select_deselect_all_button);

        //Verify Clear-Filter Functionality-Weed Type
        await productListingScreen.verifySelectAllFilterFunctionality(
            productListingScreen.weed_type_select_deselect_all_button);
