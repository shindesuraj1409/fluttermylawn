...
class LawnProblemsList extends StatelessWidget {
  final _categories = ProductCategory.values
      .where((element) => element.type == 'problems_filter')
      .toList();

  @override
  Widget build(BuildContext context) {
    return Container(
      child: _buildLawnProblemsList(),
    );
  }

  Widget _buildLawnProblemsList() {
    return ListView.builder(
      physics: NeverScrollableScrollPhysics(),
      shrinkWrap: true,
      itemBuilder: (ctx, index) {
        return _buildLawnProblemsTile(index);
      },
      itemCount: _categories.length,
    );
  }

  PlpTileWidget _buildLawnProblemsTile(int index) {
