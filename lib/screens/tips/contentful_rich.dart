...
class ContentfulRichText {
  ...
  Widget nodeToWidget(dynamic node) {
    final fl = flatten(node);
    String url;
    var isYoutube = false;
    fl.forEach((key, value) {
      if (value.toString().contains(RegExp(r'youtube.com'))) {
        url = value.toString();
        isYoutube = true;
      }
    });
    ...
    if (isYoutube) {
      node['nodeType'] = 'youtube';
      node['url'] = url;
      final Next nextNode = (nodes) => nodeListToWidget(nodes);
  ...
  dynamic _processInlineNode(
    ...
    if (node['nodeType'] == 'hyperlink' || uri?.isNotEmpty == true) {
      ...
      final String link = uri ?? node['data']['uri'];
    ...
    final nodeValue = HtmlUnescape().convert(textNode.value);
